...
var OBJLoader2 = {};
if (OBJLoader2 === undefined) {
  OBJLoader2 = {};
}

if (LoaderSupport === undefined)
  console.error(
    '"LoaderSupport" is not available. "OBJLoader2" requires it. Please include "LoaderSupport.js" in your HTML.'
  );
...
OBJLoader2 = function(manager) {
  ...
  this.manager = LoaderSupport.Validator.verifyInput(
    manager,
...
OBJLoader2.prototype = {
  ...
  setModelName: function(modelName) {
    this.modelName = LoaderSupport.Validator.verifyInput(
      modelName,
      this.modelName
    );
  },
  ...
  setStreamMeshesTo: function(streamMeshesTo) {
    this.loaderRootNode = LoaderSupport.Validator.verifyInput(
      streamMeshesTo,
      this.loaderRootNode
    );
  },
  ...
  _setCallbacks: function(callbacks) {
    ...
    if (LoaderSupport.Validator.isValid(callbacks.onLoad))
  ...
  onProgress: function(type, text, numericalValue) {
    ...
    if (LoaderSupport.Validator.isValid(this.callbacks.onProgress))
  ...
  _loadObj: function(
    ...
    // fast-fail
    if (
      !LoaderSupport.Validator.isValid(resource.url) ||
      LoaderSupport.Validator.isValid(resource.content)
    ) {
      fileLoaderOnLoad(
        LoaderSupport.Validator.isValid(resource.content)
          ? resource.content
  ...
  parseAsync: function(content, onLoad) {
    ...
    if (scope.terminateWorkerOnLoad)
  ...
  _loadMtl: function(
    ...
    if (MTLLoader === undefined)
      console.error(
        '"MTLLoader" is not available. "OBJLoader2" requires it for loading MTL files.'
      );
    ...
    if (
      ...
      crossOrigin = LoaderSupport.Validator.verifyInput(
        crossOrigin,
        "anonymous"
      );
      mtlLoader.setCrossOrigin(crossOrigin);
      mtlLoader.setResourcePath(resource.resourcePath || resource.path);
      if (LoaderSupport.Validator.isValid(materialOptions))
...
OBJLoader2.Parser.prototype = {
  ...
  setMaterials: function(materials) {
    this.materials = LoaderSupport.Validator.verifyInput(
      materials,
      this.materials
    );
  ...
  parse: function(arrayBuffer) {
    ...
    for (
      var code, word = "", bufferPointer = 0, slashesCount = 0, i = 0;
      i < length;
      i++
    ) {
  ...
  parseText: function(text) {
    ...
    for (
      var char, word = "", bufferPointer = 0, slashesCount = 0, i = 0;
      i < length;
      i++
    ) {
      char = text[i];
  ...
  buildFace: function(faceIndexV, faceIndexU, faceIndexN) {
    ...
    if (this.useIndices) {
      ...
      var indicesPointer = this.rawMesh.subGroupInUse.indexMappings[
        mappingName
      ];
  ...
  buildMesh: function(result) {
    ...
    for (var oodIndex in meshOutputGroups) {
      ...
      if (
        !LoaderSupport.Validator.isValid(materialOrg) &&
      ...
      if (this.logging.enabled && this.logging.debug) {
        var materialIndexLine = LoaderSupport.Validator.isValid(
