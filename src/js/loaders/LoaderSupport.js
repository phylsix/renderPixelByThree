...
LoaderSupport.Callbacks.prototype = {
  ...
  setCallbackOnProgress: function(callbackOnProgress) {
    this.onProgress = LoaderSupport.Validator.verifyInput(
      callbackOnProgress,
      this.onProgress
    );
  },
  ...
  setCallbackOnReportError: function(callbackOnReportError) {
    this.onReportError = LoaderSupport.Validator.verifyInput(
  ...
  setCallbackOnMeshAlter: function(callbackOnMeshAlter) {
    this.onMeshAlter = LoaderSupport.Validator.verifyInput(
      callbackOnMeshAlter,
      this.onMeshAlter
    );
  },
  ...
  setCallbackOnLoad: function(callbackOnLoad) {
    this.onLoad = LoaderSupport.Validator.verifyInput(
      callbackOnLoad,
      this.onLoad
    );
  },
  ...
  setCallbackOnLoadMaterials: function(callbackOnLoadMaterials) {
    this.onLoadMaterials = LoaderSupport.Validator.verifyInput(
...
LoaderSupport.ResourceDescriptor = function(url, extension) {
  ...
  this.name = LoaderSupport.Validator.verifyInput(
    this.name,
    "Unnamed_Resource"
  );
...
LoaderSupport.ResourceDescriptor.prototype = {
  ...
  setResourcePath: function(resourcePath) {
    this.resourcePath = LoaderSupport.Validator.verifyInput(
      resourcePath,
      this.resourcePath
    );
  }
...
LoaderSupport.MeshBuilder = function() {
  console.info(
    "Using LoaderSupport.MeshBuilder version: " +
      LoaderSupport.MeshBuilder.LOADER_MESH_BUILDER_VERSION
  );
...
LoaderSupport.MeshBuilder.prototype = {
  ...
  init: function() {
    ...
    var defaultVertexColorMaterial = new THREE.MeshStandardMaterial({
      color: 0xdcf1ff
    });
  ...
  _setCallbacks: function(callbacks) {
    if (this.validator.isValid(callbacks.onProgress))
    ...
    if (this.validator.isValid(callbacks.onMeshAlter))
      this.callbacks.setCallbackOnMeshAlter(callbacks.onMeshAlter);
    if (this.validator.isValid(callbacks.onLoad))
  ...
  buildMeshes: function(meshPayload) {
    ...
    bufferGeometry.addAttribute(
      ...
      new THREE.BufferAttribute(
        new Float32Array(meshPayload.buffers.vertices),
        3
      )
    );
    if (this.validator.isValid(meshPayload.buffers.indices)) {
      bufferGeometry.setIndex(
        new THREE.BufferAttribute(
          new Uint32Array(meshPayload.buffers.indices),
          1
        )
      );
    }
    ...
    if (haveVertexColors) {
      bufferGeometry.addAttribute(
        "color",
        new THREE.BufferAttribute(
          new Float32Array(meshPayload.buffers.colors),
          3
        )
      );
    }
    if (this.validator.isValid(meshPayload.buffers.normals)) {
      bufferGeometry.addAttribute(
        "normal",
        new THREE.BufferAttribute(
          new Float32Array(meshPayload.buffers.normals),
          3
        )
      );
    } else {
  ...
  updateMaterials: function(materialPayload) {
    ...
    if (
      this.validator.isValid(materials) &&
    ...
    if (
      ...
      for (materialName in materials) {
        ...
        if (this.logging.enabled)
          console.info(
            'Material with name "' + materialName + '" will be added.'
          );
        this.materials[materialName] = material;
      }
    }
...
LoaderSupport.WorkerSupport = function() {
  console.info(
    "Using LoaderSupport.WorkerSupport version: " +
      LoaderSupport.WorkerSupport.WORKER_SUPPORT_VERSION
  );
  this.logging = {
    enabled: true,
    debug: false
...
LoaderSupport.WorkerSupport.prototype = {
  ...
  validate: function(
    ...
    var userWorkerCode = functionCodeBuilder(
      LoaderSupport.WorkerSupport.CodeSerializer
    );
    userWorkerCode += "var Parser = " + parserName + ";\n\n";
    userWorkerCode += LoaderSupport.WorkerSupport.CodeSerializer.serializeClass(
      runnerImpl.runnerName,
      runnerImpl
    );
    ...
    var scope = this;
    if (
      LoaderSupport.Validator.isValid(libLocations) &&
      libLocations.length > 0
    ) {
      var libsContent = "";
      var loadAllLibraries = function(path, locations) {
        if (locations.length === 0) {
          scope.loaderWorker.initWorker(
...
LoaderSupport.WorkerSupport.LoaderWorker.prototype = {
  ...
  setCallbacks: function(meshBuilder, onLoad) {
    this.callbacks.meshBuilder = LoaderSupport.Validator.verifyInput(
      meshBuilder,
      this.callbacks.meshBuilder
    );
    this.callbacks.onLoad = LoaderSupport.Validator.verifyInput(
      onLoad,
...
LoaderSupport.WorkerRunnerRefImpl.runnerName =
...
LoaderSupport.NodeWorkerRunnerRefImpl.prototype = Object.create(
  LoaderSupport.WorkerRunnerRefImpl.prototype
);
LoaderSupport.NodeWorkerRunnerRefImpl.prototype.constructor =
  LoaderSupport.NodeWorkerRunnerRefImpl;
LoaderSupport.NodeWorkerRunnerRefImpl.runnerName =
  "LoaderSupport.NodeWorkerRunnerRefImpl";
...
LoaderSupport.WorkerSupport.NodeLoaderWorker.prototype.constructor =
  LoaderSupport.WorkerSupport.NodeLoaderWorker;
 ...
 * @inheritdoc
 */
LoaderSupport.WorkerSupport.NodeLoaderWorker.prototype.initWorker = function(
  code,
  runnerImplName
) {
  var supportError = this.checkSupport();
...
LoaderSupport.WorkerDirector = function(classDef) {
  console.info(
    "Using LoaderSupport.WorkerDirector version: " +
      LoaderSupport.WorkerDirector.LOADER_WORKER_DIRECTOR_VERSION
  );
  this.logging = {
    enabled: true,
    debug: false
  ...
  if (!LoaderSupport.Validator.isValid(classDef))
    throw "Provided invalid classDef: " + classDef;
...
LoaderSupport.WorkerDirector.prototype = {
  ...
  prepareWorkers: function(globalCallbacks, maxQueueSize, maxWebWorkers) {
    if (LoaderSupport.Validator.isValid(globalCallbacks))
      this.workerDescription.globalCallbacks = globalCallbacks;
    this.maxQueueSize = Math.min(
      maxQueueSize,
      LoaderSupport.WorkerDirector.MAX_QUEUE_SIZE
    );
    this.maxWebWorkers = Math.min(
      maxWebWorkers,
      LoaderSupport.WorkerDirector.MAX_WEB_WORKER
    );
    ...
    for (var instanceNo = 0; instanceNo < this.maxWebWorkers; instanceNo++) {
      ...
      workerSupport.setForceWorkerDataCopy(
        this.workerDescription.forceWorkerDataCopy
      );
  ...
  _kickWorkerRun: function(prepData, supportDesc) {
    ...
    supportDesc.workerSupport.setTerminateRequested(
      supportDesc.terminateRequested
    );
    ...
    var wrapperOnLoad = function(event) {
      ...
      if (validator.isValid(prepDataCallbacks.onLoad))
        prepDataCallbacks.onLoad(event);
    ...
    var wrapperOnProgress = function(event) {
      if (validator.isValid(globalCallbacks.onProgress))
        globalCallbacks.onProgress(event);
      if (validator.isValid(prepDataCallbacks.onProgress))
        prepDataCallbacks.onProgress(event);
    };
    ...
    var wrapperOnReportError = function(errorMessage) {
      ...
      if (validator.isValid(prepDataCallbacks.onReportError))
        continueProcessing = prepDataCallbacks.onReportError(
          supportDesc,
          errorMessage
        );
  ...
  _buildLoader: function(instanceNo) {
    ...
    if (!loader.hasOwnProperty("instanceNo"))
      throw classDef.name + ' has no property "instanceNo".';
    loader.instanceNo = instanceNo;

    if (!loader.hasOwnProperty("workerSupport")) {
      throw classDef.name + ' has no property "workerSupport".';
    }
    if (typeof loader.run !== "function")
      throw classDef.name + ' has no function "run".';
    if (
      !loader.hasOwnProperty("callbacks") ||
      !LoaderSupport.Validator.isValid(loader.callbacks)
    ) {
      console.warn(
        classDef.name +
          ' has an invalid property "callbacks". Will change to "LoaderSupport.Callbacks"'
      );
  ...
  _deregister: function(supportDesc) {
    if (LoaderSupport.Validator.isValid(supportDesc)) {
      ...
      if (this.logging.enabled)
        console.info(
          "Requested termination of worker #" + supportDesc.instanceNo + "."
        );
  ...
  tearDown: function(callbackOnFinishedProcessing) {
    ...
    this.instructionQueuePointer = this.instructionQueue.length;
    this.callbackOnFinishedProcessing = LoaderSupport.Validator.verifyInput(
